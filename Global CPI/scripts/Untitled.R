...
df <- readRDS("~/Dropbox/Clients/Master data prep/Global CPI/Company datasets/Vessel.rds") %>% mutate(ResponseId = as.character(ResponseId)) %>% # make sure the colnames of df align with the global dataset
  ...
  add_column(Company = "Vessel")

prev_global <- readRDS("~/Dropbox/Clients/Master data prep/Global CPI/global sets/current/global_Teal_8.5.20_2640.rds") ## PREVIOUS/LATEST GLOBAL DATASET

global_Vessel_8.6.20_2656 <- full_join(df, prev_global, copy=TRUE) # %>%
...
#  CHANGE NAME TWICE AND SAVE
 saveRDS(global_Vessel_8.6.20_2656, "~/Dropbox/Clients/Master data prep/Global CPI/global sets/current/global_Vessel_8.6.20_2656.rds")
 
 
 global <- global_Vessel_8.6.20_2656
 ...
 write_csv(means, "~/Dropbox/Clients/Master data prep/Global CPI/global means/new.current/global_Vessel_8.6.20_2656.csv")
