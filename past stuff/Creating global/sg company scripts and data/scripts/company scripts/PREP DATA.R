### EDITS are mostly 
   #  1.  in the import stage including through df_prep1.1
   #  2.  df_prep3  make level/order changes here.
library(tidyverse)
library(forcats)
library(lubridate)
...
company <- "Lamons"
computer <- "~/Dropbox/Clients/"
#computer <- "C:/Users/Brent/Dropbox/Clients/"
source(paste0(computer, company, "/CPI/scripts-materials/scripts/file names.R"))
#source(paste0(computer, company, "/CPI/scripts-materials/scripts/file names.R"))
...
source(paste0(file.scripts.prep, "vars.R"))
...
columns <- c("ResponseId",	"StartDate",	"EndDate",	"Status",	"Contact ID",	"Legacy Comments",	"Comments",	"Language",	"Referer",	"SessionID",	
             "User Agent",	"Tags",	"IP Address",	"Longitude",	"Latitude",	"Country",	"City",	"State/Region",	"Postal",
             
             "demo1_TENURE",  "demo2_FUNCTION", "demo3_LEVEL", "demo4_LOCATION", "demo5_GENERATION",	"demo6_GENDER",	   
...
columns[26:92] <- paste0(columns[26:92], "_", all_items)
...
df_prep1.1_columns <- df_prep1_columns %>% 
   select(ResponseId:EndDate, `cpi_Direction_vision_1_The company's vision is very clear.`:nps,
          demo1_TENURE:demo6_GENDER) %>%
   mutate_at(vars(starts_with("demo")), as.factor) 


#### get list of demo vars and remove "demo" prefixes and remove gender....
demo_var_names_raw <- colnames(df_prep1.1_columns)[grep("demo", colnames(df_prep1.1_columns))]   # this section could be improved but works 
...
#demo_vars <- demo_vars[demo_vars != "LOCATION"]
saveRDS(demo_vars, paste0(file.i.final, "demo_vars.rds"))


df_prep3_column_names_set <- df_prep2_test_filter_real_demo_as_factor  %>%
   mutate(demo1_TENURE = factor(demo1_TENURE, levels = c("Less than a year", "1 - 4 years", "5 - 8 years", "9 - 12 years"), ordered = TRUE),
          demo5_GENERATION = factor (demo5_GENERATION, levels =c("Silent Generation (born between 1925 - 1945)","Baby Boomer (born between 1946 - 1965)",
                                         "Generation X  (born between 1966 - 1980)", "Generation Y (born between 1981 - 2000)",
                                         "Generation Z (born between 2001 - 2020)"), ordered = TRUE)) %>%
          mutate(demo5_GENERATION = fct_recode(demo5_GENERATION, "Generation X (born between 1966 - 1980)" = "Generation X  (born between 1966 - 1980)")) %>%
           mutate(demo1_TENURE = fct_recode(demo1_TENURE, "9+ years" = "9 - 12 years"))
...
##### BEFORE RUNNING MAKE SURE ITEMS BEGIN ON COLUMN 4.  FUNCTION APP_DATA CHANGES COLNAMES[4:70]
df_app <- app_data(df)

  # write_csv(df_app, paste0(file.i.final, company,"_df_app.csv"))
# df_tm <- readRDS("C:/Users/Brent/Dropbox/Clients/Lamons/CPI/input/final/df.rds") %>%
#    filter(LEVEL == "Team Member")
# 
# df_tmr <- readRDS("C:/Users/Brent/Dropbox/Clients/Lamons/CPI/input/final/df.rds") %>%
#    mutate(LEVEL =  fct_recode(LEVEL, 
#                           "Other" = "Leadership (Director and up)", 
#                           "Other" = "Management", 
#                           "Other" = "Supervisor"))

# write_csv(df_tm, paste0(file.i.final, "df_tm.csv"))
# saveRDS(df_tm, paste0(file.i.final, "df_tm.rds"))
# 
# write_csv(df_tmr, paste0(file.i.final, "df_tmr.csv"))
# saveRDS(df_tmr, paste0(file.i.final, "df_tmr.rds"))
...
# df_long_tmr <- readRDS("C:/Users/Brent/Dropbox/Clients/Lamons/CPI/input/final/df_tmr.rds") %>%
#    pivot_longer(`cpi_Direction_vision_1_The company's vision is very clear.`:`extra_eff_effort_2_I am willing to put in a great deal of effort beyond what is normally expected in order to help the organization.`,
#                 names_to = "quantitative") %>%
#    separate(quantitative, into = c("cpi_or_not", "quadrant", "dimension", "question_number", "item"), sep = "_") %>%
#    mutate(quadrant = factor(quadrant, levels = c("Direction", "Operations", "People", "Engagement", "pos", "sat", "eff")),
...
#    select(-contains("oid")) 
# 
# df_long_tm <- readRDS("C:/Users/Brent/Dropbox/Clients/Lamons/CPI/input/final/df_long.rds") %>%
#    filter(LEVEL == "Team Member")
# 
# saveRDS(df_long_tm, paste0(file.i.final, "df_long_tm.rds"))
# write_csv(df_long_tm, paste0(file.i.final, "df_long_tm.csv"))
# 
# saveRDS(df_long_tmr, paste0(file.i.final, "df_long_tmr.rds"))
# write_csv(df_long_tmr, paste0(file.i.final, "df_long_tmr.csv"))

############################ ADD TO GLOBAL #######################

#   prev_global <- readRDS("~/Dropbox/Clients/Global CPI/global sets/current/global_Mercury_Fund_2.19.19.rds") ## PREVIOUS/LATEST GLOBAL DATASET
# 
# ## BELOW CHUNK: FIRST VARIABLE NAME, ADD COLUMN, MUTATE AT LAST LINE
#  global_North_Star_3.12.20 <- left_join(df %>% mutate(ResponseId = as.character(ResponseId)) %>% # make sure the colnames of df align with the global dataset
#                                             select(ResponseId,`extra_sat_satisfaction_1_Taking everything into consideration, I am satisfied with my job as a whole.`,
#                                                    `extra_eff_effort_2_I am willing to put in a great deal of effort beyond what is normally expected in order to help the organization.`,
#                                                    nps, nps_group:Engagement_sd) %>%
...
#                                             add_column(Company = "North_Star"),
#                                 prev_global, copy=TRUE) %>%  # name the new one here
#                                   bind_rows(., global) %>%
#                                   mutate(t_factor = factor(ifelse(Company == "North_Star", "North_Star", "Global")))
# # 

# CHECK TOTAL N
# # DOUBLE CHECK t_factor, CHANGE NAME HERE
#  global_North_Star_3.12.20 %>%  
#      count(t_factor)
# count


#  CHANGE NAME TWICE AND SAVE
# saveRDS(global_North_Star_3.12.20, "~/Dropbox/Clients/Global CPI/global sets/current/global_North_Star_3.12.20.rds")
...
#source(paste0(file.scripts.plots, "demo plots VERTICAL.R"))
...
#source(paste0(file.scripts.analyses, "anovas2.R"))
