...
colnames(Moody) <- Columns
Moody <- Moody %>% select(ResponseID, Company, CPI, StartDate,EndDate,      cpi_vv_1:effort,nps,Tenure, Generation, Gender, All, Role)
...
## FILTER MISSING Moody
Moody <- filter(Moody, Positivity >= 0)
...
#saveRDS(Moody, file = "~/Dropbox/Clients/Moody/make_plots/Moody.rds")
#write_csv(Moody, "~/Dropbox/Clients/Moody/analyses/Moody.csv")
saveRDS(Moody, paste0(file,"Cleaned/Moody.rds"))
write_csv(Moody, paste0(file,"Cleaned/Moody.csv"))
